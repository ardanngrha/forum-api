...
describe('CommentRepositoryPostgres', () => {
  ...
  describe('verifyOwner function', () => {
    it('should verify comment owner correctly when owner and commentId matched', async () => {
      ...
      // Action & Assert
      await expect(
        commentRepository.verifyCommentOwner(owner, commentId),
    ...
    it('should throw not found error when a comment with given id not found', async () => {
      ...
      // Action & Assert
      await expect(
        commentRepository.verifyCommentOwner(commentId, owner),
  ...
  describe('isCommentExist function', () => {
    it('should throw not found error when a comment with given id not found', async () => {
      ...
      await expect(commentRepository.isCommentExist(commentId)).rejects.toThrow(
        NotFoundError,
      );
    });
  ...
  describe('getThreadComments function', () => {
    it('should return thread comments correctly', async () => {
      ...
      // Action
      const commentDetails = await commentRepository.getThreadComments(
        'thread-123',
      );
      ...
      expect(commentDetails[0]).toHaveProperty('username', 'ardanngrha');
  ...
  describe('likeComment function', () => {
    it('should persist like comment correctly', async () => {
      ...
      // Action
      await commentRepository.likeComment(likeComment);

      // Assert
      const likes = await LikeCommentsTableTestHelper.findLikeComment({
        commentId: 'comment-123',
        owner: 'user-123',
      });
  ...
  describe('unlikeComment function', () => {
    it('should delete like comment correctly', async () => {
      ...
      // Action
      await commentRepository.unlikeComment('user-123', 'comment-123');
  ...
  describe('verifyLike function', () => {
    it('should return true when like exists', async () => {
      ...
      // Action & Assert
      const isLiked = await commentRepository.verifyLike('user-123', 'comment-123');
  ...
  describe('getLikeCount function', () => {
    it('should return correct like count', async () => {
      ...
      // Action
      const likeCount = await commentRepository.getLikeCount('comment-123');
